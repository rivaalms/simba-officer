         ...
         <u-card>
            <div class="grid gap-4 place-items-start justify-items-stretch">
               ...
               <div class="flex gap-4 items-center">
                  <img
                     :src="school?.supervisor?.user?.profile_picture || 'https://avatars.githubusercontent.com/u/739984?v=4'"
                     class="rounded-full h-16 w-16 object-cover"
                  >

                  <div class="flex flex-col">
                     <p class="text-gray-700 font-medium">
                        {{ school?.supervisor?.user?.name }}
                     </p>
...
const { data: school, pending: loading, refresh } = await useLazyAsyncData('school', () => getSchoolDetails(schoolId.value))

const { data: students, pending: studentsLoading, refresh: fetchStudents } = await useLazyAsyncData('students', () => getStudents(filter.value as API.Request.Query.Student).then(resp => mapStudents(resp)), { watch: [ () => filter.value.year ] })

const { data: teachers, pending: teachersLoading, refresh: fetchTeachers } = await useLazyAsyncData('teachers', () => getTeachers(filter.value as API.Request.Query.Teacher).then(resp => mapTeachers(resp)), { watch: [ () => filter.value.year ] })
...
const { data: studentsGrowth, pending: studentsGrowthLoading, refresh: fetchStudentsGrowth } = await useLazyAsyncData('students-growth', () => getStudentsGrowth(schoolId.value, growthFilter.value), { watch: [ () => growthFilter.value.start_year, () => growthFilter.value.end_year ] })

const { data: teachersGrowth, pending: teachersGrowthLoading, refresh: fetchTeachersGrowth } = await useLazyAsyncData('teachers-growth', () => getTeachersGrowth(schoolId.value, growthFilter.value), { watch: [ () => growthFilter.value.start_year, () => growthFilter.value.end_year ] })
